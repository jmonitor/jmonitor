...
- **Errors:** Sentry is force-disabled in self-hosted (`CloudOnlyEnvVarProcessor` empties `SENTRY_DSN`); prod errors go by email instead, driven by `ERROR_MAIL_TO` (NullHandler if empty). The `deprecation` channel is silenced there the same way (`App\Logging\DeprecationHandlerFactory`) — a self-hoster cannot act on a deprecation of our dependencies, and it buried the install output in `docker compose logs`.
...
- **Changelog:** any user-visible change (feature, behavior change, fix, upgrade step) gets a line under `## [Unreleased]` in `CHANGELOG.md`, suffixed with `(cloud)` / `(self-hosted)` when edition-specific. **A line is one sentence with no explanatory tail — invoke the `changelog-entry` skill before writing one**, it holds the rules and the entries that were sent back as too long. Dependency bumps, refactors, CI changes and internal hardening are excluded. The test is what a user would notice, not what the diff touches: a fix nobody ever hit — a crash on a payload no client sends, a validation hole closed before it bit anyone — is an internal change and stays out, however real the bug was in the code. A change to the admin panel (`admin.<domain>`) is suffixed `(self-hosted)`: cloud users never reach it. The file is not just documentation: the `Dashboard:WhatsNew` card renders the released entries in the app, and those suffixes are what hides a line from the other edition (`App\Changelog\ChangelogReader`). **Cutting a release is a maintainer task with its own procedure — read [docs/releasing.md](../docs/releasing.md) before tagging anything.**
