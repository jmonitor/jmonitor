...
#[AsTaggedItem(Metric::CaddyRequestSize->value)]
class CaddyRequestSize implements TimeSeriesMetricInterface, OptionsAwareMetricInterface
{
    use BagAwareTrait;

    public function getMetric(): Metric
    {
        return Metric::CaddyRequestSize;
    }
    ...
    public function configureTimeSerie(TimeSerieDto $lineDto, array $options = []): void
    {
        ...
        $currentValue = $this->getCaddyBag()?->requestSizePerSec->getFloat($handler) !== null
            ? Bytes::parse($this->getCaddyBag()->requestSizePerSec->getFloat($handler))->perSecond()
            : null;

        $lineDto
            ->setCurrentValue($currentValue?->roundFinalValue(2), $currentValue?->getUnit())
            ->setMeasurement('caddy_handler')
            ->setFields(['request_size_bytes_sum' => ''])
