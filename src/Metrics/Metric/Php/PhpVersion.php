...
#[AsTaggedItem(Metric::PhpVersion->value)]
class PhpVersion implements BasicMetricInterface
{
    use BagAwareTrait;
    ...
    {
        return Metric::PhpVersion;
    }

    public function configureBasicDto(BasicDto $dto, array $options = []): void
    {
        $dto
            ->setValueAvailable($this->getPhpBag()?->phpVersion !== null)
            ->setValue($this->getPhpBag()->phpVersion)
    ...
    {
        $cycle = $this->endOfLife->getCycle($value, 'https://endoflife.date/api/php.json');
